...
function DealCard({ lead, onAIClick, onDragStart, onMoveNext, stages }: DealCardProps) {
  ...
  return (
    <div
      className="glass-card rounded-xl p-3 card-hover cursor-grab active:cursor-grabbing group relative overflow-hidden"
      draggable
      onDragStart={() => onDragStart(lead)}
      onClick={() => onAIClick(lead)}
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <p className="font-semibold text-sm leading-tight text-foreground">{lead.full_name}</p>
        {isStale && <AlertCircle className="size-3.5 text-foreground/50 shrink-0 mt-0.5" aria-label="Stale deal" />}
      </div>
      <div className="flex items-center gap-1.5 mb-3">
        <div className={`size-5 rounded-full ${getAvatarColor(lead.full_name)} flex items-center justify-center`}>
          ...
          >
            {stages.map(stage => {
              ...
              return (
                <div
                  key={stage}
                  className={`rounded-xl p-3 min-w-[180px] border transition-all ${cfg.bg} ${
                    isOver ? `${cfg.border} scale-[1.01] shadow-md` : 'border-transparent'
                  }`}
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {stageValue > 0 && (
                      <Badge variant="outline" className="text-[10px] font-medium">{formatValue(stageValue)}</Badge>
                    )}
                  </div>
                  <div className="space-y-2">
                    ...
                    {stageLeads.length === 0 && (
                      <div className={`py-8 text-center text-xs text-muted-foreground border border-dashed rounded-lg transition-all ${
                        isOver ? 'border-foreground bg-foreground/5 text-foreground' : 'border-border/50'
                      }`}>
